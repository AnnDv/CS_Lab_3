...
PATH = '/Users/anadvorac/Documents/FAF/anul3/security and criptografy/laboratory3/'
PATH_POLICIES = PATH + 'Audit Files/'
...
root = Tk()
root.title('Laboratory 3 - Dvorac Ana')
...
def new_file():
    ...
    # Update status bars
    root.title('New File - Dvorac Ana')
...
def open_file():
    ...
    if extension == 'audit':
        ...
        custom_item['MACOSX_DEFAULTS_READ'] = {'type': [], 'description': [], 'regex': [], 'plist_item': [],
                                               'plist_name ': [], 'plist_option': []}
            ...
            for element in list(general_custom_item.keys()):
                ...
                if content_type_block.find(element) != -1:
                    general_custom_item[element].append(content_type_block[content_type_block.find(
                        element + ':') + length_of_element: content_type_block[content_type_block.find(
                        element + ':') + length_of_element:].find('\n') + content_type_block.find(
                        element + ':') + length_of_element].strip())
                else:
